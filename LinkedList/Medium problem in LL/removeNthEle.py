...
def DeleteNthNodefromEnd_optimal(head, N):
...
# Function to delete the Nth node from the end of the linked list
def DeleteNthNodefromEnd_brute(head, N):
    if head is None:
        return None
    cnt = 0
    temp = head
...
head = DeleteNthNodefromEnd_optimal(head, N)
printLL(head)

head2 = DeleteNthNodefromEnd_brute(head, N)
printLL(head2)